...
public class WorldListeners implements Listener{
	...
	public WorldListeners(boolean events){
		this.events = events;
	}

	@EventHandler(priority = EventPriority.LOWEST)
	public void onItemSpawn(ItemSpawnEvent e){
		List<String> disabled = MiscActionHandler.ITEM_SPAWN.getDisabled(e.getEntity().getWorld().getName());
		String id = Integer.toString(e.getEntity().getItemStack().getTypeId());
		String data = new StringBuilder(id).append(":").append(e.getEntity().getItemStack().getData().getData()).toString();
		boolean block = disabled.contains(id) || disabled.contains(data) || disabled.contains("*");
		if(block){
			if(this.events){
				BlockItemSpawnEvent event = new BlockItemSpawnEvent(e.getEntity());
				Bukkit.getPluginManager().callEvent(event);
				if(!event.willBlock())
					return;
			}
				e.setCancelled(true);
				String message = new StringBuilder().append(ChatColor.RED).append("[GriefBeGone]").append(" An illegal item spawned: ").append(e.getEntity().getType().toString())
	...
	public void onEntitySpawn(CreatureSpawnEvent e){
		...
		boolean block = MiscActionHandler.ENTITY_SPAWN.getDisabled(e.getLocation().getWorld().getName()).contains(name);
		if(block){
			if(this.events){
				BlockCreatureSpawnEvent event = new BlockCreatureSpawnEvent(e.getEntity());
				Bukkit.getPluginManager().callEvent(event);
				if(!event.willBlock())
					return;
			}
				e.setCancelled(true);
				String message = new StringBuilder().append(ChatColor.RED).append("[GriefBeGone]").append(" An illegal creature spawned: ").append(e.getEntity().getType().toString())
						.append(" at x:").append(e.getLocation().getBlockX()).append(" y:").append(e.getLocation().getBlockY()).append(" z:").append(e.getLocation().getBlockZ())
						.toString(); 
				GriefBeGone.getInstance().getLogger().info(message);
				for(Player player:Bukkit.getOnlinePlayers())
	...
	public void onItemDispense(final BlockDispenseEvent e){
		boolean[] boolArray = ActionHandler.shouldBlockAndDeleteDispense(e.getItem(), e.getBlock().getWorld().getName());
		if(boolArray[0]){
			if(this.events){
				BlockItemDispenseEvent event = new BlockItemDispenseEvent(boolArray[1], e.getBlock(), e.getItem());
				Bukkit.getPluginManager().callEvent(event);
				if(!event.willBlock()){
					return;
				}
				...
				if(boolArray[1] && e.getBlock() instanceof InventoryHolder){
					new BukkitRunnable(){
						@Override
						public void run() {
							InventoryHolder block = (InventoryHolder) e.getBlock();
							block.getInventory().remove(e.getItem().getTypeId());
							String message = new StringBuilder().append(ChatColor.RED).append("[GriefBeGone]").append(" An illegal item was dispensed: ")
		...
		new BukkitRunnable(){
			...
			public void run() {
				...
				for(ItemStack item:stand.getInventory().getContents()){
					...
					String data = new StringBuilder(id).append(":").append(item.getData().getData()).toString();
					if(disabled.contains(id) || disabled.contains(data) || disabled.contains("*")){
						if(WorldListeners.this.events){
							BlockBrewEvent event = new BlockBrewEvent(stand.getInventory());
							Bukkit.getPluginManager().callEvent(event);
							if(!event.willBlock())
								return;
						}
							stand.getInventory().setContents(preInv);
							String message = new StringBuilder().append(ChatColor.RED).append("[GriefBeGone]").append(" An illegal item was brewed: ").append(item.getType()
		...
		switch(e.getEntityType()){
			case CREEPER:
				if(MiscActionHandler.CREEPER.getDisabled(e.getLocation().getWorld().getName()) != null){
					if(this.events){
						BlockCreeperExplodeEvent event = new BlockCreeperExplodeEvent(e.blockList(), (Creeper) e.getEntity());
						Bukkit.getPluginManager().callEvent(event);
						if(!event.willBlock())
							return;
					}
						e.blockList().clear();
						String message = new StringBuilder().append(ChatColor.RED).append("[GriefBeGone]").append(" A creeper exploded at x:").append(" at x:")
			...
			case FIREBALL:
				if(MiscActionHandler.FIREBALL.getDisabled(e.getLocation().getWorld().getName()) != null){
					if(this.events){
						BlockFireballExplodeEvent event = new BlockFireballExplodeEvent(e.blockList(), (Fireball) e.getEntity());
						Bukkit.getPluginManager().callEvent(event);
						if(!event.willBlock())
							return;
					}
						e.blockList().clear();
						String message = new StringBuilder().append(ChatColor.RED).append("[GriefBeGone]").append(" A fireball exploded at x:").append(" at x:")
								.append(e.getLocation().getBlockX()).append(" y:").append(e.getLocation().getBlockY()).append(" z:").append(e.getLocation().getBlockZ())
								.toString();
						GriefBeGone.getInstance().getLogger().info(message);
						for(Player player:Bukkit.getOnlinePlayers())
			...
			case PRIMED_TNT:
				if(MiscActionHandler.TNT.getDisabled(e.getLocation().getWorld().getName()) != null){
					if(this.events){
						BlockTnTExplodeEvent event = new BlockTnTExplodeEvent(e.blockList(), (TNTPrimed) e.getEntity());
						Bukkit.getPluginManager().callEvent(event);
						if(!event.willBlock())
							return;
					}
						e.blockList().clear();
						String message = new StringBuilder().append(ChatColor.RED).append("[GriefBeGone]").append(" TnT exploded at x:").append(" at x:")
								.append(e.getLocation().getBlockX()).append(" y:").append(e.getLocation().getBlockY()).append(" z:").append(e.getLocation().getBlockZ())
								.toString();
						GriefBeGone.getInstance().getLogger().info(message);
						for(Player player:Bukkit.getOnlinePlayers())
